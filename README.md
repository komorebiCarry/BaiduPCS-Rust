...
### 🔁 分享同步（Share Sync）🆕
- ✅ **订阅第三方分享链接**，周期/定时拉取文件列表，与上次快照对比差异
- ✅ **自动应用变更**到网盘目录（转存）和/或本地目录（下载）
- ✅ **三种冲突策略**：
  - `overwrite`（覆盖式）：直接覆盖目标中的同名文件
  - `versioned`（新版本式）：保留旧文件为 `name(YYYYMMDD-HHMMSS).ext`，写入新文件
  - `skip`（跳过）：目标已存在则不处理
- ✅ **多目标并行**：一条订阅可同时配置"网盘 + 本地"双目标
- ✅ **可调轮询节奏**：固定间隔（≥10 分钟）或每日定时；支持启用/停用开关
- ✅ **删除行为可控**：默认**保留**目标中的旧文件；可开启"分享者删除 → 目标也删除"
- ✅ **完整审计**：每次运行记录 added/modified/removed/failed 计数 + 每个文件级 run_item
- ✅ **实时推送**：通过 WebSocket 广播 DiffDetected / RunStarted / RunCompleted 等事件
- ✅ **REST API**：`/api/v1/share-sync/subscriptions[/:id[/runs|/trigger|/snapshots/latest]]`
- ✅ **Web UI**：`/share-sync` 页面，订阅列表 + 创建/编辑表单 + 运行历史 + 实时进度
