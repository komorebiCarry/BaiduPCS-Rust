...
impl BackupRecordManager {
    ...
    fn init_database(&self) -> Result<()> {
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_snapshots_config_remote_path
             ON encryption_snapshots(config_id, remote_path, is_directory)",
            [],
        )?;
    ...
    /// 批量写入加密映射。
    ///
    /// 文件夹扫描会在内存中完成全部映射解析，最后在一个事务中写入新增或
    /// 变更的映射，避免把“每个文件一次连接/一次事务”放在扫描和上传前面。
    pub fn add_snapshots_batch(&self, snapshots: &[EncryptionSnapshot]) -> Result<usize> {
    ...
    /// 一次加载指定配置下的全部文件映射。
    ///
    /// 文件夹扫描使用此批量接口，然后在内存中按文件名和路径消歧，避免
    /// 每个本地文件都单独获取 SQLite 连接、准备 SQL 并执行一次查询。
    pub fn find_file_snapshots_by_config(
        &self,
        config_id: &str,
    ) -> Result<Vec<EncryptionSnapshot>> {
        let conn = self.get_conn()?;
        let mut stmt = conn.prepare(&format!(
            "SELECT {SNAPSHOT_SELECT_COLUMNS}
             FROM encryption_snapshots
             WHERE config_id = ?1 AND is_directory = 0
             ORDER BY updated_at DESC, id DESC"
        ))?;

        let rows = stmt.query_map(params![config_id], snapshot_from_row)?;
        rows.collect::<rusqlite::Result<Vec<_>>>()
            .map_err(Into::into)
    }
    ...
    pub fn find_snapshots_by_encrypted_names(&self, encrypted_names: &[String]) -> Result<Vec<EncryptionSnapshot>> {
        ...
        let rows = stmt.query_map(params.as_slice(), snapshot_from_row)?;
    ...
    pub fn get_all_folder_mappings_by_encrypted_name(&self, encrypted_name: &str) -> Result<Vec<EncryptionSnapshot>> {
        ...
        let rows = stmt.query_map(params![encrypted_name], snapshot_from_row)?;
    ...
    fn test_add_snapshots_batch_persists_complete_mapping() {
        ...
        let snapshots = vec![
            EncryptionSnapshot {
                ...
                version: 1,
                key_version: 1,
                remote_path: "/backup/docs/one.age".to_string(),
                remote_name: "one.age".to_string(),
                is_directory: false,
            ...
            EncryptionSnapshot {
                ...
                version: 1,
                key_version: 1,
                remote_path: "/backup/docs/two.age".to_string(),
                remote_name: "two.age".to_string(),
                is_directory: false,
                status: "completed".to_string(),
            },
        ];

        assert_eq!(record_manager.add_snapshots_batch(&snapshots).unwrap(), 2);
        let stored = record_manager
            .find_file_snapshots_by_config("config-1")
            .unwrap();
        assert_eq!(stored.len(), 2);
        assert_eq!(stored[0].local_path, "/home/user/docs/two.txt");
        assert_eq!(stored[0].remote_name, "two.age");
        assert_eq!(stored[0].file_size, 20);
    }
}
