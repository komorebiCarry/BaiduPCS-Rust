...
//!
//! ## 模块地图（本文件 ~10K 行，按主题定位）
//!
//! | 行号区间       | 主题                          | 说明 |
//! |---------------|------------------------------|------|
//! | 35-200        | struct AutoBackupManager     | 字段 + 构造 |
//! | 200-700       | CRUD：config / task           | 创建、更新、删除 |
//! | 700-1500      | 扫描 + 调度                   | 配合 scheduler/ 子目录 |
//! | 1500-3000     | 执行循环                      | 配合 sync/ 子目录 |
//! | 3000-4500     | 上传 / 下载 / 加密            | 配合 task/ encryption/ |
//! | 4500-5000     | 调试 + 健康检查               | 健康检查已抽到 `health.rs` |
//! | 5000-7000     | WebSocket 事件                | 状态广播 |
//! | 7000-9000     | 加密 / 持久化辅助             | 配合 encryption/ persistence/ |
//! | 9000-10000    | 任务恢复 / 优雅关闭            | shutdown / 持久化 |
//! | 10000-10473   | 数据结构 + 测试                | HealthCheckResult 等已外迁 |
//!
//! ## 大文件拆分路线图（已部分完成）
//!
//! ✅ `health.rs`（健康检查）       — 网络探测 + 磁盘检查
//! ⏳ 计划中：执行循环可拆为 `executor.rs`；WS 事件可拆为 `events_impl.rs`
//!
//! ## 跨文件依赖
//!
//! 真正的业务逻辑（调度策略、变更聚合、文件监听、上传执行、加密快照）
//! 已下沉到 `scheduler/`、`sync/`、`watcher/`、`record/`、`encryption/` 等子模块；
//! 本文件只承担"协调"职责，因此仍然偏长。

use super::config::*;
use super::health::{
    check_disk_space as health_check_disk_space, check_network_reachable, HealthCheckResult,
};
...
impl AutoBackupManager {
                ...
                .or_insert_with(|| {
                    ...
                    tokio::spawn(async move {
                        task_loop(ctrl_clone, || {
                            let m = manager.clone();
                            let c = cfg.clone();
                            async move { m.execute_backup_for_config(&c).await }
                        })
                        .await;
                    });
    ...
    ///
    /// 检查系统各组件的运行状态。
    /// 实际探测逻辑已下沉到 `super::health`，本方法只负责串联各组件状态。
    pub async fn health_check(&self) -> HealthCheckResult {
        ...
        // 真实网络探测（TCP 443，3 秒超时）
        let network_ok = check_network_reachable().await;

        // 磁盘空间（基于 temp_dir 是否存在）
        let disk_space_ok = health_check_disk_space(&self.temp_dir);
    ...
    async fn execute_backup_for_config(&self, config: &BackupConfig) -> anyhow::Result<()> {
        ...
        if let Some(transferring_task) = self
            .tasks
            .iter()
                ...
                if let Some(ref cfg) = config {
                    if is_upload {
                        ...
                        let actual_local_path = self
                            .get_download_task_local_path(transfer_task_id)
                            .await
                        ...
                        let fs_id = match update.fs_id {
                            ...
                            None => self
                                .get_download_task_fs_id(transfer_task_id)
                                .await
    ...
    async fn create_upload_tasks_for_files(
        ...
        for file_task in file_tasks.iter_mut() {
            ...
            if let Some(ref existing_upload_id) = file_task.related_task_id {
                ...
                // 🔥 关键修复：恢复的上传任务初始状态为 Paused，必须主动启动/恢复
                // 否则任务会一直停留在 Paused 状态，不会自动执行
                if let Err(e) = upload_mgr.start_task(existing_upload_id).await {
                    tracing::warn!(
                        "启动已恢复的上传任务失败，尝试 resume: file_task={}, upload_task={}, error={}",
                        file_task_id, existing_upload_id, e
                    );
                    // start_task 失败可能是因为状态不对（如 Paused），尝试 resume
                    if let Err(e2) = upload_mgr.resume_task(existing_upload_id).await {
                        tracing::error!(
                            "恢复上传任务也失败: file_task={}, upload_task={}, error={}",
                            file_task_id,
                            existing_upload_id,
                            e2
                        );
    ...
    async fn execute_sync_backup(
        ...
        if plan.uploads.is_empty() && plan.downloads.is_empty() && plan.state_updates.is_empty() {
            ...
            self.tasks.remove(task_id);
            // 通知前端移除该任务
            Self::publish_status_changed_static(&self.ws_manager, task_id, "preparing", "removed");
            return Ok(());
        }
