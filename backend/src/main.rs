...
async fn main() -> anyhow::Result<()> {
    ...
    let api_routes = Router::new()
        ...
        .route("/auth/accounts", get(handlers::list_accounts))
        .route("/auth/accounts/:uid/switch", post(handlers::switch_account))
        .route("/auth/accounts/:uid", delete(handlers::remove_account))
        ...
        .route(
            "/share-sync/preview-tree",
            post(handlers::share_sync::preview_tree),
        )
        // 🔥 加密数据导出 API
