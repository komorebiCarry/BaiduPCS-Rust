...
use crate::uploader::{
    ...
    UploadChunkManager, UploadChunkScheduler, UploadTask, UploadTaskScheduleInfo, UploadTaskStatus,
};
...
impl UploadManager {
    ...
    pub async fn override_task_owner_uid(
        ...
        let task_arc = self
            .tasks
            .get(task_id)
            .map(|task_info| task_info.task.clone());
    ...
    pub async fn create_batch_tasks(
        ...
        // 普通批量上传，不是文件夹上传
        self.create_batch_tasks_internal(files, encrypt, false, conflict_strategy, None)
            .await
    }
    ...
    pub async fn create_batch_tasks_with_owner(
        ...
        self.create_batch_tasks_internal(
            files,
            encrypt,
            false,
            conflict_strategy,
    ...
    pub async fn create_folder_task<P: AsRef<Path>>(
        ...
        let task_ids = self
            .create_batch_tasks_internal(tasks, encrypt, true, None, None)
            .await?;
    ...
    pub async fn delete_task(&self, task_id: &str) -> Result<()> {
        ...
        let (is_backup, slot_id, was_active, task_owner_uid_raw) =
    ...
    pub async fn create_batch_tasks_dedup(
        ...
        self.create_batch_tasks_dedup_internal(
            files,
            encrypt,
            is_folder_upload,
            conflict_strategy,
                                ...
                                let (actual_local_path, actual_total_size) = if encrypt_enabled {
                                    ...
                                    .await
                                    {
                                        Ok(encrypted_path) => {
                                            // 获取加密后文件大小
                                            let encrypted_size =
                                                match tokio::fs::metadata(&encrypted_path).await {
                                                    Ok(m) => m.len(),
                                                    Err(e) => {
